...
function Subscription() {
  const [individualsTeamsPlanList, setIndividualsTeamsPlanList] = useState<any>(null);
  const [schoolsPlanList, setSchoolsPlanList] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIndividualsTeamsPlanList(data?.individualsTeamsPlanList)
        setSchoolsPlanList(data?.schoolsPlanList)
      } catch (error) {
      ...
      <div className='2xl:py-20 py-11'>
        <div className='container'>
          <div className='flex flex-col gap-10 md:gap-20'>
            ...
            {/* Individuals/Teams Section */}
            <div className='flex flex-col gap-6'>
              <h3 className='text-2xl text-dark_black/60 dark:text-white/60 text-center font-light'>For Individuals/Teams</h3>
              <div className='grid grid-cols-1 xxl:grid-cols-2 gap-6'>
                {individualsTeamsPlanList?.map((items: any, index: number) => (
                  <div
                            ...
                            return (
                              <li key={index} className='flex items-center gap-3'>
                                <Image
                                  ...
                                  width={20}
                                  height={20}
                                  className={items.plan_bg_color === 'bg-dark_black' ? 'brightness-0 invert' : ''}
                                />
            ...
            {/* Schools Section */}
            <div className='flex flex-col gap-6'>
              <h3 className='text-2xl text-dark_black/60 dark:text-white/60 text-center font-light'>For Schools</h3>
              <div className='grid grid-cols-1 xxl:grid-cols-2 gap-6'>
                {schoolsPlanList?.map((items: any, index: number) => (
                  <div
                    className={`${items.plan_bg_color} p-6 md:p-10 rounded-2xl`}
                    key={index}>
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
